# coding=utf-8
"""
packaging script for the qgis_section project
...
zipname = "qgis-section"
zipext = ".zip"

def run_tests():
    out, err = subprocess.Popen(["python", "-m", "qgis_section.test"]).communicate()
    if err :
...
def deploy(zip_filename, new_version):
    ...
    xmlfile = os.path.join(repository_dir, "qgis_section.xml")
    with open(xmlfile, 'r') as file:
        data = file.readlines()
    data[3]= "  <pyqgis_plugin name='qgis_section' version='" + str(new_version) + "'>\n"
            ...
            if not re.match(r".*(test_data|docs).*", root):
                for file_ in files:
                    if re.match(r".*\.(w15|py|txt|ui|json|sql|png|svg|qml)$", file_) \
                        ...
                        fake_root = root.replace(base_dir, "qgis_section")
